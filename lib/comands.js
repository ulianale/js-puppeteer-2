...
module.exports = {
  ...
  selectionFreePlace: async function (page, row) { // выбрав любой ряд, функция сама выберет свободное место
    let x = 0; // счетчик
  ...
  selectionBusyPlace: async function (page, row) {  // выбрав ряд, функция сама выберет НЕдоступное место 
    // функция БЕЗ "throw new Error", так как ряд будет тот же самый, где уже есть забронированые места
    await page.waitForSelector(".buying-scheme__wrapper");
    for (let i = 1; i < 11; i++) {
      let place = `div.buying-scheme__wrapper > div:nth-child(${row}) > span:nth-child(${i})`;
      let classExist = await page.$eval(place,
